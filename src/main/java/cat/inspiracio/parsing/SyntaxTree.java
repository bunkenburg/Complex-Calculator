...
public class SyntaxTree {
    ...
    private SyntaxTree getFactor(String s, ParsePosition parseposition, ParsePosition parseposition1) throws ParseException {
        Object obj = null;
        int i;
        ...
        } else if((i = recogniseFunction(s, parseposition, parseposition1)) != -1) {
        ...
        } else if(in(s.charAt(parseposition.getIndex()), "0123456789"))
            obj = readDigits(s, parseposition, parseposition1);
        else if(in(s.charAt(parseposition.getIndex()), "iep\u03C0\u221E"))
            obj = readConstant(s, parseposition, parseposition1);
        else if(in(s.charAt(parseposition.getIndex()), "zx"))
    ...
    public SyntaxTree parse(String s)throws ParseException{
        s = stripBlanks(s);
        return parse(s, 0, s.length());
    }

    private SyntaxTree parse(String s, int i, int j)throws ParseException{
        if(i >= j)
        ...
        if((k = last('+', s, i, j)) > -1)
            ...
            for(
                    obj = getFactor(s, parseposition, parseposition1);
                    parseposition.getIndex() != parseposition1.getIndex();
                    obj = new SyntaxTreeBinary(2, ((SyntaxTree) (obj)), getFactor(s, parseposition, parseposition1))
            );
        }
        return ((SyntaxTree) (obj));
    }
    ...
    private SyntaxTree readConstant(String s, ParsePosition parseposition, ParsePosition parseposition1) throws ParseException{
        Complex ec = null;
    ...
    private SyntaxTree readDigits(String s, ParsePosition parseposition, ParsePosition parseposition1) throws ParseException {
        int i;
    ...
    private SyntaxTree bracketBin(String s, int i, int j, int k, int l) throws ParseException {
    ...
    private SyntaxTree bracketPlusMinus(String s, int i, int j, int k, int l) throws ParseException {
        if(j == i) {
    ...
    protected Complex abs(Complex c){
        double d = cat.inspiracio.complex.package$.MODULE$.abs(c);
        return Real(d);
    }

    protected Complex Im(Complex c){
        double d = cat.inspiracio.complex.package$.MODULE$.Im(c);
        return Real(d);
    }

    protected Complex Re(Complex c){
        double d = cat.inspiracio.complex.package$.MODULE$.Re(c);
        return Real(d);
    }

    protected Complex argument(Complex c){
        if( c instanceof cat.inspiracio.complex.imp.CartesianComplex){
            cat.inspiracio.complex.imp.CartesianComplex cc = (cat.inspiracio.complex.imp.CartesianComplex)c;
            double d = cc.argument();
            return Real(d);
        } else {
