...
class RiemannSphereTest extends FunSuite {
  ...
  test("angles 0"){
    val z = plane2angles(0)
    assert( z === (π,0) )
  }

  test("angles 1"){
    val z = plane2angles(1)
    assert( z === (π/2,0) )
  }

  test("angles i"){
    val z = plane2angles(i)
    assert( z === (π/2, π/2) )
  }

  test("angles2plane(0,0)"){
    val z = angles2plane(0,0)
    assert( z === ∞ )
  }

  test("angles2plane(π,0)"){
    val z = angles2plane(π,0)
    assert( z === 0 +- 1e-15 )
  }

  test("angles2plane(π/2,0)"){
    val z = angles2plane(π/2,0)
    assert( z === 1 +- 1e-15)
  }

  test("angles2plane(π/2, π/2)"){
    val z = angles2plane(π/2, π/2)
    assert( z === i +- π/180)
  }
